...
export async function assertOrderAccess(order: PgOrder, actor: AuthActor) {
  ...
  if (isStaffLike(actor) && actor.staffId) {
    ...
    // RSM and ASM see orders through their Sales Managers, so keep note access
    // aligned with the orders they can already see in the list view.
    ...
    if (actor.role === "ASM") {
      ...
      if (await isOrderInActorScope({ role: "staff", actorId: actor.staffId.toString(), isAsm: true }, order.id.toString())) return;
    }
  }
