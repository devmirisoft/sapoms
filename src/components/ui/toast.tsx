...
function Toast({ className, swipeDirection = ["up", "right"], ...props }: ToastPrimitive.Root.Props) {
  return (
    <ToastPrimitive.Root
      data-slot="toast"
...
// react-toastify-shaped shim so existing call sites keep working
type NotifyOptions = { autoClose?: number }
const push =
  (type?: string) =>
  (title: React.ReactNode, opts?: NotifyOptions) =>
    toast.add({ title, type, timeout: opts?.autoClose })

const notify = Object.assign(push(), {
